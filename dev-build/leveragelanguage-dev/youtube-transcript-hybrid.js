...
(function() {
  ...
  let captionCollection = {
    ...
    whisper: {
      ...
      isRecording: false,
      chunkStartTime: 0,
      chunkDuration: 5, // ✅ BALANCED: 5-second chunks for quality while preventing freezing
      chunkGap: 5, // ✅ BALANCED: 5-second gaps for good cleanup without over-fragmentation
  ...
  function startContinuousAudioRecording() {
    ...
    // ✅ SIMPLE SAFETY STOP: Auto-stop after 1 minute to prevent any freezing
    const emergencyStopTimer = setTimeout(() => {
      if (captionCollection.isCollecting) {
        console.log('🛑 SAFETY STOP: Auto-stopping after 1 minute to prevent freezing');
        console.log('💡 Recording complete! You can start a new recording for more content');
        stopCaptionCollection();
        
        // Send completion message to sidepanel
        chrome.runtime.sendMessage({
          action: 'transcriptSafetyStop',
          reason: 'Completed 1-minute safe recording window',
          duration: 60000,
          segments: captionCollection.segments.length,
          canRestart: true
        }).catch(err => console.log('Failed to send safety stop message:', err));
      }
    }, 60000); // 60 seconds - balanced duration with comprehensive state reset protection
  ...
  function cleanAutoGeneratedText(text) {
    ...
    console.log('🧽 Enhanced cleaning auto-generated text (fixing word fragments):', text.substring(0, 60) + '...');
    
    // ENHANCED: Fix word fragmentation issues from chunk boundaries + essential contractions
    let cleaned = text
      // ✅ ENHANCED: Fix common word fragmentations seen in user's transcript
      .replace(/\ba\s+re\s+you\b/gi, "are you")
      .replace(/\ba\s+re\b/gi, "are")
      .replace(/\bthe\s+re\b/gi, "there")
      .replace(/\bthe\s+n\b/gi, "then") 
      .replace(/\bthe\s+se\b/gi, "these")
      .replace(/\ba\s+sk\b/gi, "ask")
      .replace(/\ba\s+sking\b/gi, "asking")
      .replace(/\ba\s+bout\b/gi, "about")
      .replace(/\ba\s+lex\b/gi, "Alex")  // Common name fragmentation
      .replace(/\ba\s+lot\b/gi, "a lot")
      ...
      .replace(/\ba\s+ny\b/gi, "any")
      .replace(/\ba\s+nd\b/gi, "and")
      .replace(/\bo\s+ver\b/gi, "over")
      .replace(/\bo\s+kay\b/gi, "okay")
      .replace(/\be\s+ven\b/gi, "even")
      .replace(/\be\s+very\b/gi, "every")
      .replace(/\be\s+nough\b/gi, "enough")
      .replace(/\bw\s+ith\b/gi, "with")
      .replace(/\bw\s+hen\b/gi, "when")
      .replace(/\bw\s+here\b/gi, "where")
      .replace(/\bw\s+hat\b/gi, "what")
      .replace(/\bu\s+nder\b/gi, "under")
      .replace(/\bu\s+p\b/gi, "up")
      .replace(/\bi\s+n\b/gi, "in")
      .replace(/\bo\s+f\b/gi, "of")
      .replace(/\bt\s+o\b/gi, "to")
      .replace(/\bf\s+or\b/gi, "for")
      .replace(/\bb\s+ut\b/gi, "but")
      .replace(/\bs\s+o\b/gi, "so")
      
      // Fix contractions (existing ones)
    ...
    if (cleaned !== text && cleaned.length > 5) {
      console.log('✨ Enhanced cleaned result (word fragments fixed):', cleaned.substring(0, 60) + '...');
    }
