...
(function() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'startCaptionCollection') {
      ...
      // ✅ NEW: Detect caption type and adapt collection strategy
      const captionType = detectCaptionType();
      console.log('🎯 Caption type detected:', captionType);
      
      if (captionType === 'auto-generated') {
        // Use larger chunks and stream processing for auto-captions
        console.log('🤖 Using auto-caption collection strategy');
        ...
        console.log('📝 Using manual caption collection strategy');
  ...
  function processAutoGeneratedCaption(currentText, timestampInSeconds, extractedTimestamp, videoId, youtubeLink, isNewChunk) {
    // ✅ Enhanced: Process auto-generated captions with aggressive cleaning
    console.log('🤖 Processing auto-generated caption:', currentText.substring(0, 40) + '...');
    ...
    if (newContent && newContent.length > 2) {
      ...
      console.log('📝 Added to stream:', bufferAddition);
      console.log('📚 Current stream buffer:', captionCollection.streamBuffer.substring(0, 80) + '...');
      ...
      if (shouldCreateSegment(captionCollection.streamBuffer, isNewChunk)) {
        ...
        if (cleanedText.length > 8) {
          ...
          console.log('✅ Created auto-generated segment:', cleanedText.substring(0, 50) + '...');
  ...
  function extractNewContentFromStream(currentText) {
    ...
    if (cleanedContent !== currentText && cleanedContent.length > 0) {
      console.log('🔄 Extracted new content:', cleanedContent, '(removed', bestOverlapLength, 'overlapping words)');
    }
  ...
  function shouldCreateSegment(streamBuffer, isNewChunk) {
    ...
    if (streamBuffer.length > 60) {
      ...
      if (endsWithNaturalBreak) {
        console.log('📝 Creating segment at natural break');
        return true;
      }
    }
    ...
    if (streamBuffer.length > 150) {
      console.log('📏 Creating segment due to length limit');
      return true;
    }
    ...
    if (words.length >= 12 && streamBuffer.length > 80) {
      console.log('📊 Creating segment due to word count (', words.length, 'words)');
      return true;
    }
  ...
  function cleanAutoGeneratedText(text) {
    ...
    let cleaned = text
      ...
      // Enhanced merging fixes based on user feedback
      ...
      .replace(/\b(\w+)(where|when|what|who|why|how)\b/gi, (match, p1, p2) => {
        if (p1.length > 2 && !['somewhere', 'anywhere', 'everywhere'].includes(match.toLowerCase())) {
          return p1 + ' ' + p2;
        }
        return match;
      })
      .replace(/\b(\w+)(really|should|understand|change|revenue|week|month|year|day)\b/gi, (match, p1, p2) => {
        if (p1.length > 1 && !['already', 'actually'].includes(match.toLowerCase())) {
          return p1 + ' ' + p2;
        }
        return match;
      })
  ...
  function masterCleanAutoGeneratedSegment(text) {
    // Master cleaning function specifically for auto-generated segments
    console.log('🧽 Master cleaning segment:', text.substring(0, 50) + '...');
    ...
    cleaned = cleanAutoGeneratedText(cleaned);
    
    // Pass 2: Advanced repetition removal
    cleaned = removeAdvancedRepetitions(cleaned);
    
    // Pass 3: Fragment cleanup
    cleaned = cleanFragments(cleaned);
    
    // Pass 4: Final validation and polish
    cleaned = finalPolish(cleaned);
    
    if (cleaned !== text) {
      console.log('✨ Master clean result:', cleaned.substring(0, 50) + '...');
    }
          ...
          if (isUnique && isNotSubstring) {
            ...
            if (captionCollection.currentChunkStartTime !== undefined && 
              ...
              if (captionCollection.autoGenerated && 
                ...
                console.log('🔄 Forcing new chunk due to full stream buffer');
                isNewChunk = true;
              }
            }
            ...
            // ✅ NEW: Handle auto-generated captions differently
            if (captionCollection.autoGenerated) {
              // Stream processing for auto-generated captions with enhanced overlap detection
              processAutoGeneratedCaption(currentText, timestampInSeconds, extractedTimestamp, videoId, youtubeLink, isNewChunk);
            } else {
              // Enhanced processing for manual captions with better duplicate detection
              const isDuplicate = captionCollection.segments.some(segment => 
                Math.abs(segment.start - timestampInSeconds) < 2 && 
                segment.text.toLowerCase().includes(currentText.toLowerCase().substring(0, 20))
              );
