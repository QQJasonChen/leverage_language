...
(function() {
  ...
  function processAutoGeneratedCaption(currentText, timestampInSeconds, extractedTimestamp, videoId, youtubeLink, isNewChunk) {
    // ✅ NEW: Process auto-generated captions with stream reconstruction
    console.log('🤖 Processing auto-generated caption:', currentText.substring(0, 40) + '...');
    
    // Extract new content from overlapping stream
    const newContent = extractNewContentFromStream(currentText);
    
    if (newContent && newContent.length > 2) {
      // Add to stream buffer
      captionCollection.streamBuffer += (captionCollection.streamBuffer ? ' ' : '') + newContent;
      captionCollection.lastProcessedText = currentText;
      
      console.log('📝 Added to stream:', newContent);
      ...
      if (shouldCreateSegment(captionCollection.streamBuffer, isNewChunk)) {
        const cleanedText = cleanAutoGeneratedText(captionCollection.streamBuffer);
  ...
  function cleanAutoGeneratedText(text) {
    // Clean up auto-generated text for better readability
    return text
      .replace(/\s+/g, ' ') // Normalize spaces
