...
class AIService {
  constructor() {
    ...
    this.trialManager = null; // Trial API manager for alpha testing
  }
  ...
  async initialize() {
    try {
      // Initialize trial manager for alpha testing
      if (typeof TrialAPIManager !== 'undefined') {
        this.trialManager = new TrialAPIManager();
        await this.trialManager.initialize();
        console.log('🎯 Trial API Manager initialized for alpha testing');
      }
  ...
  buildProfessionalCoreSections(text, language, langName, isWord, features, level) {
    ...
    if (features.grammarAnalysis) {
      ...
      if (!isWord) {
        ...
        // Additional sentence structure breakdown for TMP languages
        if (language === 'dutch') {
          prompt += `\n**🔍 特別說明「${text}」的荷蘭語TMP結構分析：**\n`;
          prompt += `請逐字分析句子結構，重點關注Time-Manner-Place詞序規則：\n`;
          prompt += `\n**📋 TMP分析範例格式：**\n`;
          prompt += `以「ik ga morgen met mijn vrienden naar het park」為例：\n`;
          prompt += `- **主語 (Subject)**: Ik (第一位)\n`;
          prompt += `- **動詞 (Verb)**: ga (第二位，符合V2規則)\n`;
          prompt += `- **⏰ Time**: morgen (時間副詞，放在動詞後)\n`;
          prompt += `- **🚶 Manner**: met mijn vrienden (方式/陪伴，介詞片語)\n`;
          prompt += `- **📍 Place**: naar het park (地點，方向介詞+目的地)\n`;
          prompt += `\n**現在請按此TMP模式分析給定句子，特別說明：**\n`;
          prompt += `1. 每個成分在句中的確切位置\n`;
          prompt += `2. 為什麼采用這種順序排列\n`;
          prompt += `3. 是否符合標準荷蘭語TMP規則\n`;
          prompt += `4. 與中文/英文語序的對比差異\n`;
        } else if (language === 'german') {
          ...
          prompt += `\n**🔍 特別說明「${text}」的日語SOV結構分析：**\n`;
          prompt += `請分析句子的SOV語序和助詞使用，特別說明：\n`;
          prompt += `1. **逐詞解析**：每個詞彙的讀音（平假名）、漢字寫法（如有）、詞性、意思\n`;
          prompt += `2. **主語、賓語、動詞的確切位置**：分析SOV語序結構\n`;
          prompt += `3. **助詞系統解析**：は/が、を、に、で、よ等助詞的選擇邏輯和語法功能\n`;
          prompt += `4. **修飾語的位置和作用**：形容詞、副詞等修飾語的位置規則\n`;
          prompt += `5. **敬語層次分析**：敬語、謙讓語、丁寧語的使用適當性和場合\n`;
          prompt += `6. **語尾變化分析**：動詞活用、形容詞變化等語法變化\n`;
          prompt += `7. **句型分類**：判斷句型類型（陳述句、疑問句、命令句、否定句等）\n`;
        }
      } else {
    ...
    // 5. Similar Example Sentences - User's favorite feature!
    prompt += `### 🎯 類似例句 - 延伸練習\n`;
    prompt += `**請提供 6-8 個使用相同語法結構的練習句（用戶特別喜歡這個功能！）：**\n\n`;
    prompt += `請用以下格式呈現：\n`;
    prompt += `1. [${langName}例句] → [中文翻譯]\n`;
    prompt += `2. [${langName}例句] → [中文翻譯]\n`;
    prompt += `3. [${langName}例句] → [中文翻譯]\n`;
    prompt += `4. [${langName}例句] → [中文翻譯]\n`;
    prompt += `5. [${langName}例句] → [中文翻譯]\n`;
    prompt += `6. [${langName}例句] → [中文翻譯]\n`;
    prompt += `7. [${langName}例句] → [中文翻譯]（進階）\n`;
    prompt += `8. [${langName}例句] → [中文翻譯]（進階）\n\n`;
    prompt += `請確保例句涵蓋不同場景：日常生活、工作、學習、社交等。\n\n`;
  ...
  isSimpleText(text) {
    ...
    // Never use simple analysis for Japanese - Japanese needs detailed word and structure analysis
...
📝 **中文翻譯**：[翻譯]
🧩 **逐詞解釋**：[每個重要詞的意思]
📖 **語法結構**：[句型分析]${this.getLanguageSpecificStructure ? this.getLanguageSpecificStructure(language) : (language === 'dutch' ? '\n🇳🇱 **荷蘭語結構解析**：\n   - V2語序：動詞在第二位的規則\n   - 分離動詞：前綴分離的邏輯\n   - 詞彙位置：時間、地點、賓語排列\n   - 語句類型：主句、從句結構特點' : '')}
🗣️ **發音要點**：[重音和語調]
🎯 **類似例句 - 延伸練習**：
**請提供 6-8 個使用相同語法結構的練習句（用戶特別喜歡這個功能！）：**
1. [${langName}例句] → [中文翻譯]
2. [${langName}例句] → [中文翻譯]
3. [${langName}例句] → [中文翻譯]
4. [${langName}例句] → [中文翻譯]
...
請適度詳細，每項2-3句話。`;
  ...
  // Get API key (user's key or trial key)
  async getAPIKey(provider) {
    // User has their own API key
    if (this.settings.apiKey) {
      return { key: this.settings.apiKey, isTrial: false };
    }
    
    // Use trial key if available
    if (this.trialManager && await this.trialManager.canUseTrial()) {
      const trialKey = this.trialManager.getTrialAPIKey(provider);
      if (trialKey) {
        return { key: trialKey, isTrial: true };
      }
    }
    
    throw new Error('No API key available. Please add your own API key or try again later.');
  }  // 調用 Gemini API
  async callGeminiAPI(prompt, complexity = 'simple') {
    ...
    try {
      ...
      // Get API key (user's own or trial key)
      const { key: apiKey, isTrial } = await this.getAPIKey('gemini');
      ...
      // Track trial usage if using trial key
      if (isTrial && this.trialManager) {
        await this.trialManager.incrementUsage();
      }
  ...
  async callOpenAIAPI(prompt, complexity = 'simple') {
    ...
    try {
      ...
      // Get API key (user's own or trial key)
      const { key: apiKey, isTrial } = await this.getAPIKey('openai');
