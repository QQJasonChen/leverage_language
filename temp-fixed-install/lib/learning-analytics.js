...
class LearningAnalytics {
  ...
  loadFromStorage(data) {
    // Convert vocabulary data and restore nested Maps
    const vocabularyData = new Map();
    if (data.vocabulary) {
      data.vocabulary.forEach(([key, value]) => {
        // Restore the actions Map from stored array or object
        if (value.actions) {
          if (Array.isArray(value.actions)) {
            value.actions = new Map(value.actions);
          } else if (typeof value.actions === 'object') {
            // Convert plain object to Map
            value.actions = new Map(Object.entries(value.actions));
          } else {
            value.actions = new Map();
          }
        } else {
          value.actions = new Map();
        }
        vocabularyData.set(key, value);
      });
    }
  ...
  async save() {
    // Convert vocabulary data with nested Maps to serializable format
    const vocabularyArray = Array.from(this.analytics.vocabulary.entries()).map(([key, value]) => {
      // Convert the actions Map to array for storage
      const serializedValue = {
        ...value,
        actions: Array.from(value.actions.entries())
      };
  ...
  recordVocabularyInteraction(word, language, action, context = {}) {
    ...
    // Record action (with safety check)
    if (!(vocabData.actions instanceof Map)) {
      // Fix corrupted actions data
      vocabData.actions = new Map();
    }
    const actionCount = vocabData.actions.get(action) || 0;
    vocabData.actions.set(action, actionCount + 1);
    
    // Update retention data for correct/incorrect answers
    if (action === 'correct_answer' || action === 'incorrect_answer') {
      vocabData.retention.reviews++;
      vocabData.retention.lastReview = now;
      
      if (action === 'correct_answer') {
        vocabData.retention.correct++;
        // Decrease difficulty slightly for correct answers
        vocabData.retention.difficulty = Math.max(0, vocabData.retention.difficulty - 0.1);
      } else {
        // Increase difficulty for incorrect answers
        vocabData.retention.difficulty = Math.min(1, vocabData.retention.difficulty + 0.15);
      }
    }
