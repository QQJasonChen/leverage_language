...
class YouTubeTranscriptFetcher {
  ...
  async restructureWithAI(text, aiService, language) {
    try {
      // Create a structured prompt for transcript restructuring
      const structuredText = `Please restructure this YouTube auto-generated transcript text into proper sentences with correct punctuation and capitalization. Keep all the original words but make them readable:
...
- Fix capitalization
- Keep the exact same words
- Remove redundant repetitions
- Make it into clear, readable sentences
- Language: ${language || 'English'}

Return only the improved text, nothing else.`;

      // Use the AI service's analysis method
      const response = await aiService.generateAnalysis(structuredText, language || 'en');
      
      // Extract just the text content if the response is structured
      ...
      console.error('❌ AI restructuring failed:', error);
