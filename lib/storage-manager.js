...
class StorageManager {
  ...
  analyzeForErrors(analysisData) {
    ...
    // Stronger correct indicators - must be definitive statements
    ...
    const isDefinitivelyCorrect = strongCorrectIndicators.some(indicator => analysisTextLower.includes(indicator));
    
    // Priority logic: Strong error indicators override weaker correct indicators
    const hasStrongErrorIndicators = hasErrorMarkers || hasErrorPatterns || detectedErrors.length > 0;
