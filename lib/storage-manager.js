...
class StorageManager {
  ...
  async ensureStorageStructure() {
    ...
    const defaults = {
      ...
      [this.STORAGE_KEYS.USER_SETTINGS]: {
        ...
        autoSave: true,
        exportFormat: 'json',
        saveAudio: false, // Disabled by default to save storage space
        maxAudioFiles: 20 // Limit audio files to prevent storage issues
      },
    ...
    // IMPORTANT: Update existing user settings if maxReports is still the old limit
    if (result[this.STORAGE_KEYS.USER_SETTINGS] && 
        result[this.STORAGE_KEYS.USER_SETTINGS].maxReports === 100) {
      console.log('🔄 Upgrading user settings: increasing maxReports from 100 to 5000');
      const updatedSettings = {
        ...result[this.STORAGE_KEYS.USER_SETTINGS],
        maxReports: this.MAX_REPORTS // Update to 5000
      };
  ...
  async saveAIReport(searchText, language, analysisData, audioData = null, videoSource = null, updateExisting = true) {
    try {
  ...
  // Export all audio data as downloadable files
  async exportAllAudioData() {
    try {
      ...
      const exportData = {
        version: '1.0',
        exportDate: new Date().toISOString(),
        totalAudioFiles: reportsWithAudio.length,
        audioFiles: reportsWithAudio.map((report, index) => ({
          id: report.id,
      ...
      return {
        ...
        totalFiles: reportsWithAudio.length,
        totalSize: this.formatBytes(totalSizeBytes)
      };
  ...
  // Remove audio data from all reports (immediate storage space saver)
  async removeAllAudioData() {
    try {
      ...
      // Remove audio data from all reports
      reportsWithAudio.forEach(report => {
        delete report.audioData;
      });
      ...
      console.log(`✅ Removed audio data from ${reportsWithAudio.length} reports`);
      
      // Update settings to disable audio saving by default
      const settings = await this.getUserSettings();
      await this.updateUserSettings({ ...settings, saveAudio: false });
      
      return { success: true, removedAudio: reportsWithAudio.length };
