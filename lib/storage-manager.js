  ...
  async saveAIReport(searchText, language, analysisData, audioData = null, videoSource = null, updateExisting = true, detectionMethod = null) {
    try {
      ...
      if (existingIndex !== -1 && updateExisting) {
        ...
        // Debug existing report update
        console.log('🔍 DEBUG UPDATE - Params:', { videoSource, analysisData });
        console.log('🔍 DEBUG UPDATE - Existing videoTimestamp before:', existingReport.videoSource?.videoTimestamp);
        ...
        if (videoSource) {
          console.log('🔍 DEBUG UPDATE - New videoSource:', videoSource);
          console.log('🔍 DEBUG UPDATE - New videoTimestamp:', videoSource.videoTimestamp);
          existingReport.videoSource = videoSource;
          console.log('🎬 Updated existing report with video source:', videoSource);
          console.log('🔍 DEBUG UPDATE - Result videoTimestamp after:', existingReport.videoSource?.videoTimestamp);
        }
        if (detectionMethod) {
        ...
        // Debug new report creation
        console.log('🔍 DEBUG NEW - Params:', { videoSource, analysisData });
        console.log('🔍 DEBUG NEW - VideoSource:', videoSource);
        console.log('🔍 DEBUG NEW - VideoTimestamp:', videoSource?.videoTimestamp);
        ...
        // Debug new report result
        console.log('🔍 DEBUG NEW - Result newReport.videoSource:', newReport.videoSource);
        console.log('🔍 DEBUG NEW - Result videoTimestamp:', newReport.videoSource?.videoTimestamp);
        ...
        // 🔧 IMPORTANT: Also save to history manager for proper history tracking
        // This ensures that AI analysis saves appear in the history tab with correct video metadata
        try {
          const historyDetectionMethod = videoSource ? 
            (videoSource.platform === 'netflix' ? 'netflix-learning' :
             videoSource.platform === 'youtube' ? 'youtube-learning' : 
             detectionMethod || 'ai-analysis') : 
            (detectionMethod || 'ai-analysis');
          
          console.log('💾 Also saving AI analysis to history with video source:', videoSource);
          
          // Send message to background.js to save to history manager
          if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
            await chrome.runtime.sendMessage({
              ...
              data: {
                text: normalizedText,
                language: normalizedLanguage,
                detectionMethod: historyDetectionMethod,
                websitesUsed: [],
                videoSource: videoSource
              }
            });
            console.log('✅ AI analysis also saved to history via background.js with video metadata');
          } else {
            console.warn('⚠️ Chrome runtime not available in storage manager context');
          }
        } catch (historyError) {
          console.error('❌ Failed to save AI analysis to history:', historyError);
