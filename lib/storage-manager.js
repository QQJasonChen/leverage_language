...
class StorageManager {
  ...
  async migrateReportsDetectionMethod() {
    try {
      ...
      reports.forEach(report => {
        if (!report.detectionMethod && report.videoSource) {
          // Determine detection method based on video source characteristics
          const videoSource = report.videoSource;
          
          if (videoSource.url && !videoSource.url.includes('youtube.com') && !videoSource.url.includes('youtu.be')) {
            // Non-YouTube URL suggests article
            report.detectionMethod = 'article-learning';
            updatedCount++;
            console.log('🔄 Migrated report to article-learning:', report.searchText.substring(0, 30) + '...');
