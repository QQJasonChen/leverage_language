...
class ErrorHandler {
  ...
  initializeRecoveryStrategies() {
    ...
    // Fix: Add missing UNKNOWN error type strategy
    this.recoveryStrategies.set(this.errorTypes.UNKNOWN, {
      retry: false,
  ...
  async executeRecoveryAction(errorInfo, strategy, context) {
    switch (strategy.action) {
      ...
      case 'log_and_continue':
        // Fix: Add missing recovery action for UNKNOWN errors
        console.log('🔧 Error logged and continuing with graceful degradation:', errorInfo.message);
        return { success: true, message: 'Error logged and continuing', recovered: false };
  ...
  async retryOperation(context) {
    ...
    if (context.operation && context.retry && typeof context.retry === 'function') {
      try {
