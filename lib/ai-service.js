...
class AIService {
  ...
  migrateOpenAIModel(savedModel) {
    // Default to gpt-5-nano for best cost efficiency (40% cheaper than gpt-4o-mini)
    if (!savedModel) {
      return 'gpt-5-nano';
    }
    ...
    if (savedModel === 'gpt-3.5-turbo') {
      console.log('🔄 Migrating from deprecated gpt-3.5-turbo to gpt-5-nano (better quality, 90% cheaper)');
      return 'gpt-5-nano';
    }
    
    // Auto-upgrade to GPT-5 models for better performance
    if (savedModel === 'gpt-4o-mini') {
      console.log('🚀 Auto-upgrading from gpt-4o-mini to gpt-5-nano (better performance, 40% cheaper)');
      return 'gpt-5-nano';
    }
    
    if (savedModel === 'gpt-4o') {
      console.log('🚀 Auto-upgrading from gpt-4o to gpt-5-mini (better performance, similar cost)');
      return 'gpt-5-mini';
    }
    
    // Keep valid GPT-5 models
    const validGPT5Models = ['gpt-5-nano', 'gpt-5-mini', 'gpt-5'];
    if (validGPT5Models.includes(savedModel)) {
      return savedModel;
    }
    
    // Keep valid GPT-4o models (for backwards compatibility)
    if (savedModel === 'gpt-4o-mini' || savedModel === 'gpt-4o') {
      return savedModel;
    }
    
    // Unknown model, default to gpt-5-nano
    console.log(`⚠️ Unknown OpenAI model "${savedModel}", defaulting to gpt-5-nano`);
    return 'gpt-5-nano';
  }
...
請適度詳細，每項2-3句話。`;
  ...
  async callOpenAIAPI(prompt, complexity = 'simple') {
    ...
    const timeoutDuration = timeouts[complexity] || 8000;
    const maxTokens = tokens[complexity] || 512;
    const modelToUse = this.settings.openaiModel || 'gpt-5-nano';
    const startTime = Date.now();
