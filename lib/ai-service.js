...
class AIService {
  ...
  buildPrompt(text, language) {
    // Try to use professional prompts if available
    if (this.settings.useProfessionalPrompts !== false) {
    ...
    // ALWAYS start with translation and simple breakdown
    prompt += `**🔤 翻譯與簡單解釋**\n\n`;
    prompt += `**中文翻譯：** 請提供準確且自然的中文翻譯\n`;
    if (!isWord) {
      prompt += `**句子分解：** 請逐詞解釋每個詞的意思，並說明整句話的重點\n`;
    } else {
      prompt += `**基本解釋：** 請提供最常用的意思和詞性\n`;
    }
    prompt += `\n---\n\n`;
