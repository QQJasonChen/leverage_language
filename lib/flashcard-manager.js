...
class FlashcardManager {
  ...
  async createEnhancedFromReport(report) {
    try {
      ...
      const flashcardData = {
        front: report.searchText,
        back: flashcardContent.translation || this.extractTranslation(report.analysis),
        definition: flashcardContent.context || this.extractDefinition(report.analysis),
        pronunciation: flashcardContent.pronunciation || this.extractPronunciation(report.analysis),
        language: report.language,
        tags: (report.tags || []).concat(['ai-enhanced']),
        // Store additional AI-generated content
        memoryTip: flashcardContent.memoryTip || '',
        aiGenerated: true,
        generatedAt: flashcardContent.timestamp
      };
